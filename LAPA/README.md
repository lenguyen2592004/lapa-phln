...
If `conda` is not installed, use a local `.venv` environment (recommended for this repo):
...
# Full training/deploy stack (slower, larger)
...
The default `requirements.txt` is CPU-safe and works on Ubuntu 22.04 (Jammy). If you want CUDA 12 GPU acceleration, install optional extras after the base install:
...
The inference entrypoint now also saves result artifacts by default:

- `outputs/inference/latent_action.json`
- `outputs/inference/latent_action_visualization.png`
- `outputs/inference/latent_action_visualization.mp4`

You can customize paths and inputs:
...
For Ubuntu 22.04 or constrained machines (missing CUDA/JAX/checkpoint issues), you can still complete the pipeline with deterministic fallback outputs:

```bash
python -m latent_pretraining.inference --allow-mock-on-error
# or force lightweight mode:
python -m latent_pretraining.inference --mock
```

If your GPU backend fails to initialize (for example, CUDA/cuDNN mismatch), the entrypoint now retries once automatically with `JAX_PLATFORMS=cpu`.

If the process is killed on CPU fallback (`Killed` in terminal), run with a smaller checkpoint streaming buffer:
...
If you see `msgpack.exceptions.BufferFull`, increase this value (for example `12` or `16`).

### Hardware Notes for LAPA-7B

- LAPA-7B checkpoint is large (~13.6GB). In practice, CPU inference needs much more than that in host RAM.
- If available memory is too small, code will now stop early with a clear error instead of crashing WSL.
- Recommended minimums for a stable run:
  - Host RAM available to WSL: at least 12GB (preferably 16GB+)
  - GPU path: CUDA-enabled JAX installed, and enough VRAM for your runtime setup

For WSL, increase memory/swap in `%UserProfile%/.wslconfig`, then run `wsl --shutdown`.

For GPU inference, install CUDA-enabled JAX wheels and keep GPU backend enabled:
...
```json
{
  ...
  "conversations": [
    ...
    },
    {
      "from": "gpt",
      "raw_actions": [
        0.0004934221506118809, -0.00011252239346504211, -0.001941084861755371,
        0.013634951062806884, 0.013678191591275368, -0.004913635449167675, 0.0
      ],
      "states": {
        "eef_pos": [
          0.24725835025310516, -0.022094586864113808, 0.9283081889152527
        ],
        "eef_euler": [
          3.1202197128587876, -0.7113159765223936, -0.10937155062330725
        ],
        "gripper_state": 0.0
      }
    }
