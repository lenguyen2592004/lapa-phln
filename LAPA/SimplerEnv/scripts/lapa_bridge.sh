...
gpu_id=0
declare -a policy_models=("lapa")
main_py="simpler_env/main_inference_lapa.py"
...
for policy_model in "${policy_models[@]}"; do
  CUDA_VISIBLE_DEVICES=${gpu_id} python ${main_py} --policy-model ${policy_model} --ckpt-path ${ckpt_path} \
    ...
    --robot-init-x-range ${robot_init_x} ${robot_init_x} 1 --robot-init-y-range ${robot_init_y} ${robot_init_y} 1 --obj-variation-mode episode --obj-episode-range 0 24 \
