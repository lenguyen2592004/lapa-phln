...
def write_video(path, images, fps=5):
    ...
    images_npy = []
    for image in images:
        frame = image.numpy() if not isinstance(image, np.ndarray) else image
        frame = np.asarray(frame)
        if frame.dtype != np.uint8:
            frame = np.clip(frame, 0, 255).astype(np.uint8)
        images_npy.append(frame)

    if media is not None:
        media.write_video(str(path), images_npy, fps=fps)
        return
    ...
    raise RuntimeError("Neither mediapy nor imageio is installed. Install one of them to write videos.")
